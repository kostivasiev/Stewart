<div class="panel-body">
  <div class="form-horizontal">
    ...
    <div class="form-group" id="role-options">
      ...
      <div class="col-md-8">
        @foreach (App\Right::where('name', '<>', 'Super Admin')->orderBy('name')->get() as $right)
            <div class="checkbox">
              <label>
                <input type="checkbox" name="rights[]" value="{{ $right->id }}" title="{{ $right->description }}" {{ !empty($company) ? $company->hasRight($right->name) ? 'checked' : '' : '' }}> {{ $right->name }}
              </label>
            </div>
        @endforeach
      </div>
